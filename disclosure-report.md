...
Add a limit on the total number of expanded items, regardless of whether the expansion is from numeric ranges or comma-separated patterns. For example:

```javascript
// In lib/expand.js, add before the append() call:
if (q.length > rangeLimit) {
  throw new RangeError('expanded array length exceeds range limit...');
}
```

Or add a maximum output size check in the `append()` function.
